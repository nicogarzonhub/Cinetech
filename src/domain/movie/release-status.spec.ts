...
describe('getReleaseStatus', () => {
  ...
  it('descarta una fecha que no se puede parsear', () => {
    expect(getReleaseStatus('no-es-una-fecha', now)).toEqual({ kind: 'unknown' });
  });
  ...
  it('no consulta el reloj del sistema: el mismo estreno cambia de estado según el "ahora" recibido', () => {
    ...
    expect(getReleaseStatus(releaseDate, new Date('2000-01-01')).kind).toBe('upcoming');
    expect(getReleaseStatus(releaseDate, new Date('2100-01-01')).kind).toBe('released');
  });
});
