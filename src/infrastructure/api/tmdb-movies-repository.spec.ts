...
describe('tmdbMoviesRepository.getTrendingThisWeek', () => {
  it('mapea una respuesta válida a MovieSummary, incluida la URL del póster', async () => {
    server.use(
      http.get(TRENDING_URL, () =>
        HttpResponse.json({
          results: [
            { id: 238, title: 'El padrino', poster_path: '/poster.jpg', release_date: '1972-03-24' },
          ],
        }),
      ),
    );
    ...
    expect(movies).toEqual([
      {
        ...
        releaseStatus: { kind: 'released', releaseDate: new Date('1972-03-24') },
      },
    ]);
  });
  ...
  it('traduce un póster ausente a `posterUrl: null`, nunca a una URL rota', async () => {
    server.use(
      http.get(TRENDING_URL, () =>
        HttpResponse.json({
          results: [{ id: 1, title: 'Sin póster', poster_path: null, release_date: '' }],
        }),
      ),
    );
  ...
  it('lanza ApiError cuando la respuesta no tiene la forma esperada, sin reventar con una afirmación de tipo', async () => {
    server.use(http.get(TRENDING_URL, () => HttpResponse.json({ results: [{ id: 'no-es-un-número' }] })));

    await expect(tmdbMoviesRepository.getTrendingThisWeek(now)).rejects.toBeInstanceOf(ApiError);
  });
  ...
  it('traduce el código 34 de TMDB ("no encontrado") sobre un 404 en un ApiError con reason "not-found"', async () => {
    ...
    await expect(tmdbMoviesRepository.getTrendingThisWeek(now)).rejects.toMatchObject({
      reason: 'not-found',
    });
  });

  it('traduce un 429 en un ApiError con reason "rate-limited"', async () => {
    server.use(http.get(TRENDING_URL, () => new HttpResponse(null, { status: 429 })));
